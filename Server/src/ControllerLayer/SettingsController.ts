...
class SettingsController {
  ...
  patchCandidatesSettings = async (
    ...
    try {
      ...
      const serviceResponse = await this.settingService.patchCandidatesSettings(
        id,
        candidates
      );
   ...
   */
  patchNumberOfAttemptsSettings = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
      ...
      const serviceResponse =
        await this.settingService.patchNumberOfAttemptsSettings(
          id,
          numberOfAttempts
        );
  ...
  /**
   * @name clearAnswer
   * @description handles incoming request for clearing the answer
   */
  clearAnswer = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ////////////////////////////////////////////////////////////////////////////////////////////////////
      /* Variables and constants declarations */
      ...
      const serviceResponse = await this.settingService.clearAnswer(id);
      ...
      response.success = true;
      response.successMsg = "answer cleared";
      response.response = serviceResponse.response;
      res.status(StatusCodes.OK).json(response);
      return;
    } catch (error: any) {
