...
class SettingsService {
	...
	public async fetchSettings(): Promise<IServiceResponse<any>> {
		...
		try {
            ...
            const random = Math.floor(Math.random()*(queryResult.response[0].number_of_attempts + 1));
            const saveAnswer = await this.settingsRepository.updateAnswer(queryResult.response[0]._id, queryResult.response[0].candidates[random]);
	 ...
	 */
    public async patchCandidatesSettings(id: string, candidates: string[]): Promise<IServiceResponse<any>> {
		const res: IServiceResponse<any> = {
			errorMessage: '',
			internalError: false,
			response: {},
	 ...
	 * @returns response object 
	 */
    public async patchNumberOfAttemptsSettings(id: string, numberOfAttempts: number): Promise<IServiceResponse<any>> {
		const res: IServiceResponse<any> = {
			errorMessage: '',
			internalError: false,
			response: {},
		...
		try {
			...
			queryResult = await this.settingsRepository.updateNumberOfattempts(id, numberOfAttempts);
